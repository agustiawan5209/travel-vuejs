...
const FormInputData = ref({
    destinasi: props.jadwal.destinasi,
    tanggal: props.jadwal.tanggal,
    waktu: props.jadwal.waktu,
    harga_tiket: props.jadwal.harga_tiket,
    kuota: props.jadwal.kuota,
});

// Watch for changes in props.jadwal and update FormInputData accordingly
watch(
  () => props.jadwal,
  (newJadwal) => {
    FormInputData.value.destinasi = newJadwal.destinasi;
    FormInputData.value.tanggal = newJadwal.tanggal;
    FormInputData.value.waktu = newJadwal.waktu;
    FormInputData.value.harga_tiket = newJadwal.harga_tiket;
    FormInputData.value.kuota = newJadwal.kuota;
  },
  { immediate: true }
);
...
const SubmitData = async () => {
    ...
    try {
        ...
        const response = await axios.post(`/api/travel/${props.jadwal.id}`, formData, {
            headers: {
...
<template>
    <div class="bg-white p-8 rounded-lg shadow-md w-full">
        <h2 class="text-lg font-semibold mb-4">Form Ubah Data</h2>
